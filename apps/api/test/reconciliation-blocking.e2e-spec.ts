...
describe('Reconciliation blocking rule (e2e)', () => {
  ...
  it('flags a sale that pushes loyalty sales over total sales, then blocks the next one outright', async () => {
    ...
    const station = await stations.create({ name: 'Mbita', code: `MBT-${randomUUID().slice(0, 6)}` });
    await prices.create(
      { product: Product.AGO, pricePerLitre: 100, effectiveFrom: new Date(Date.now() - 86_400_000).toISOString() },
      admin,
    );
    ...
    const first = await sales.createSale(
      { customerPhone: customer.phoneNumber, product: Product.AGO, amountPaid: 900, stationId: station.id, saleDate: today, idempotencyKey: randomUUID() },
      admin,
    );
    ...
    const second = await sales.createSale(
      { customerPhone: customer.phoneNumber, product: Product.AGO, amountPaid: 200, stationId: station.id, saleDate: today, idempotencyKey: randomUUID() },
      admin,
    );
    ...
    expect(record.status === ReconciliationStatus.EXCEEDED || record.status === ReconciliationStatus.NEAR_LIMIT).toBe(
      true,
    );
    ...
    await expect(
      sales.createSale(
        { customerPhone: customer.phoneNumber, product: Product.AGO, amountPaid: 50, stationId: station.id, saleDate: today, idempotencyKey: randomUUID() },
        admin,
      ),
    ).rejects.toThrow();
  });
