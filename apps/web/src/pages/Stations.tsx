      ...
      {deleting && (
        <Modal title={`Delete ${deleting.name}?`} onClose={() => !deleteBusy && setDeleting(null)}>
          <div style={{ fontSize: 13.5, color: 'var(--color-text-secondary)', lineHeight: 1.55 }}>
            This permanently deletes <strong>{deleting.name}</strong> ({deleting.code}). Only allowed if no customers, sales, sales assistants, or users still reference this station — otherwise, deactivate it instead.
          </div>
          {deleteError && (
